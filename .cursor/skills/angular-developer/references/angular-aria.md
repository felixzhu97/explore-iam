...
- **Combobox (Autocomplete)**: Applied to an ` ` element. Ideal when typing filters the list.
- **Select**: Applied to a focusable wrapper like a ` ` or ` ` element. Users select from a list of options.
...
1. **Never use native HTML elements like ` `** when asked to implement these specific Aria patterns. Use the `ng*` directives.
