...
| Scope | 范围 | OAuth scope value object; OIDC std (`openid`, `profile`, `email`) plus GitHub-style product scopes (`write:ai_chat`, `admin:chat`) | VO `Scope`; `PermissionPoint.oauthScope: Scope`; `OidcClient.scopes: Set<Scope>` → `oauth2_registered_client.scopes` | implemented |
...
| OAuth Scope Accessor | OAuth 范围访问 | Scope string granted on access tokens | Behavior | `PermissionPoint.oauthScopeValue()` / `Scope.value()` | implemented |
