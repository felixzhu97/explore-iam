...
**Exception**: Static `value` on `<input type="radio">` and `<option>` is **allowed and required** — it identifies which option the input represents, not the bound field value.
...
Do NOT do this: `<input [formField]="..." name="..." />` or `<select [formField]="..." name="...">`.
...
- **`select` binding**: You CAN bind to `<option multiple>`. Ensure options have `value` attributes.
...
| **readonly attribute** | `<input [formField]="..." readonly>` | Use `readonly()` rule in schema |
| **min/max attributes** | `<input [formField]="..." min="1" max="10">` | Use `min()` and `max()` rules in schema |
| **value binding** | `<input [formField]="..." [value]="...">` | Do NOT use `[value]` with `[formField]` (static `value` on radio/checkbox is OK) |
...
| **Checkbox Array** | `<input type="checkbox" [formField]="form.tags"` (string[]) | Checkboxes ONLY bind to `boolean` |
